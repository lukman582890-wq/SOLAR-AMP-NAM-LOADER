...
function SOLARCTRL(patch){ _solarSend({msg:'SAMFUI',msgTag:112,ctrlTag:-1,data:btoa(JSON.stringify(patch))}); }
...
function refreshFx(){SPVFUI(20,state.delay/100);SPVFUI(21,state.reverb/100);SPVFUI(23,moduleBypass.fx?0:1);const modes={DELAY:0,REVERB:1,CHORUS:2,PHASER:3,TREMOLO:4};const mode=modes[selectedFxMode]??0;SOLARCTRL({fxDelay:state.delay,fxReverb:state.reverb,fxActive:!moduleBypass.fx,fxMode:mode})}
function refreshDrive(){SPVFUI(14,state.drive/100);SPVFUI(15,state.tone/100);SPVFUI(16,state.level/100);SOLARCTRL({odDrive:state.drive,odTone:state.tone,odLevel:state.level,odActive:!moduleBypass.od})}
...
function refreshEq(){SPVFUI(17,state.eqLow/100);SPVFUI(18,state.eqMid/100);SPVFUI(19,state.eqHigh/100);SOLARCTRL({eqLow:state.eqLow,eqMid:state.eqMid,eqHigh:state.eqHigh,eqActive:!moduleBypass.eq});eqGraph.low=(state.eqLow-50)*.24;eqGraph.mid=(state.eqMid-50)*.24;eqGraph.high=(state.eqHigh-50)*.24;updateEqGraph()}
function refreshCab(){SPVFUI(8,moduleBypass.cab?0:1);SOLARCTRL({cabActive:!moduleBypass.cab})}
...
function toggleModule(name){if(!(name in moduleBypass))return;moduleBypass[name]=!moduleBypass[name];const icon=document.querySelector('.module-bypass[data-module="'+name+'"]');if(icon){icon.textContent=moduleBypass[name]?'🖕':'👍';icon.classList.toggle('bypassed',moduleBypass[name]);icon.setAttribute('aria-pressed',String(!moduleBypass[name]))}SOLARCTRL({odActive:!moduleBypass.od,eqActive:!moduleBypass.eq,cabActive:!moduleBypass.cab,fxActive:!moduleBypass.fx,ampBypass:moduleBypass.amp,namActive:namMode});if(name==='eq')SPVFUI(7,moduleBypass.eq?0:1);if(name==='cab')SPVFUI(8,moduleBypass.cab?0:1);if(name==='od')SPVFUI(22,moduleBypass.od?0:1);if(name==='fx')SPVFUI(23,moduleBypass.fx?0:1);refreshAllBypass();refreshNamBypass();refreshStatusIndicators()}
...
function applyAmpModel(name){selectedAmp=name;const p={'British 800':0,'American Clean':1,'Modern 5150':2}[name]??0;SPVFUI(25,p/2);SOLARCTRL({ampModel:p})}
...
function applyOdModel(name){selectedOd=name;const p={'Tube Screamer':[35,50,72,true],'Tight OD':[48,58,70,true],'Off':[0,50,100,false]}[name]||[35,50,72,true];state.drive=p[0];state.tone=p[1];state.level=p[2];SPVFUI(14,p[0]/100);SPVFUI(15,p[1]/100);SPVFUI(16,p[2]/100);SPVFUI(22,p[3]?1:0);moduleBypass.od=!p[3];SOLARCTRL({odDrive:p[0],odTone:p[1],odLevel:p[2],odActive:p[3]});const icon=document.querySelector('.module-bypass[data-module="od"]');if(icon){icon.textContent=moduleBypass.od?'🖕':'👍';icon.classList.toggle('bypassed',moduleBypass.od)}}
const builtInCabMap={
